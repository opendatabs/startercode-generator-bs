...
def py_literal(s: str) -> str:
    """Return a Python string literal (we use JSON since it matches Python)."""
    return json.dumps(s, ensure_ascii=False)

def create_marimo_apps(data):
    """Create marimo apps (.py) in 04_marimo/ using a safe template."""
    template_path = f"{TEMPLATE_FOLDER}template_marimo.py"
    with open(template_path, "r", encoding="utf-8") as f:
        tmpl = f.read()

    for idx in tqdm(data.index):
        identifier = data.loc[idx, "dataset_identifier"]
        id_short = identifier.split("@")[0]  # numeric part

        intro_md = f"## Open Government Data, provided by **{PROVIDER}**\n" \
                   f"*Autogenerated marimo app for* **{identifier}**"
        dataset_title_md = f"## Dataset\n# **{data.loc[idx, 'title']}**"
        links_md = "## Data set links\n\n" \
                   f"[Direct data shop link for dataset]({BASELINK_DATASHOP}{identifier})"
        metadata_md = "## Metadata\n" + data.loc[idx, "metadata"]

        # Build a filled template with *Python literals* for strings
        app_src = tmpl
        app_src = app_src.replace("{{ INTRO_MD }}", py_literal(intro_md))
        app_src = app_src.replace("{{ DATASET_TITLE_MD }}", py_literal(dataset_title_md))
        app_src = app_src.replace("{{ LINKS_MD }}", py_literal(links_md))
        app_src = app_src.replace("{{ METADATA_MD }}", py_literal(metadata_md))
        app_src = app_src.replace("{{ DOWNLOAD_URL }}",
                                  py_literal(f"{BASELINK_DATASHOP}{identifier}/download"))
